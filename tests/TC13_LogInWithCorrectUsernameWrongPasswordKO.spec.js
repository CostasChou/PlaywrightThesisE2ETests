...
test('TC13_LogInWithCorrectUsernameWrongPasswordKO', async ({ page }) => {
    //Creation of an object of LogInWebpage
    const logInWebpage =  new LogInWebpage(page);
    //Visist webpage
    logInWebpage.goToLogInWebpage();
        //Perform Log In action with WRONG PASSWORD
    logInWebpage.logIn(fixtures.username, fixtures.wrongPassword);
    ...
    const isTextPresent = pageTextContent.includes('Authentication attempt has failed, likely due to invalid credentials. Please verify and try again.');
