...
test('TC12_LogInWithWrongUsernameCorrectPasswordKO', async ({ page }) => {
    //Creation of an object of LogInWebpage
    const logInWebpage =  new LogInWebpage(page);
    //Visist webpage
    logInWebpage.goToLogInWebpage();
    //Perform Log In action with WRONG USERNAME
    logInWebpage.logIn(fixtures.wrongUsername, fixtures.password);
    ...
    const isTextPresent = pageTextContent.includes('Your account is not recognized and cannot login at this time.');
